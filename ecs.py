#
# Py3AWS - A wrapper of AWS for Python 3
# Author: Yu Yagi <yagi@yagi.sh>
# License: Python Software Foundation License
#

# Below is the original copyright declaration of PyAWS
...
__author__ = "Yu Yagi <yagi@yagi.sh>"
__version__ = "0.0.1"
...
def buildRequest(argv):
    ...
    argv['Version'] = '%s' % Meta.version
    argv['Timestamp'] = time.strftime('%Y-%m-%dT%H:%M:%SZ')
    ...
    sign = hmac.new(bytes(getSecretKey(), 'UTF-8'), bytes(req, 'UTF-8'), hashlib.sha256).digest()
    sign = urllib.parse.quote(base64.b64encode(sign))
    url = url + '&Signature=%s' % sign
    return url
...
def setSecretKey(secret_key = None):
    ...
    environment variable: AMAZON_Meta.secret_key; if no secret key is 
    set, NoSecretKey exception is raised."""
    if secret_key:
